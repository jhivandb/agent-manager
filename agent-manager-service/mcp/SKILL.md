...
Use this guide when creating AMP custom evaluators through MCP.

Workflow:
- Decide the evaluator `type`: `code` or `llm_judge`
- Decide the evaluator `level`: `trace`, `agent`, or `llm`
- Draft the full `source` using the matching section below
- Define `config_schema` so it matches the code arguments or prompt placeholders
- Call the `create_custom_evaluator` MCP tool with the completed payload
